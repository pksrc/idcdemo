...
//tealium universal tag - utag.sender.4001 ut4.0.202010230656, Copyright 2020 Tealium.com Inc. All Rights Reserved.
try {
  (function (id, loader) {
    var u = {};
    ...
    u.send = function (a, b) {
      if (u.ev[a] || u.ev.all !== undefined) {
        ...
        for (f = 0; f < u.data.multicat_arr.length; f++) {
          u.loader({ "type" : "iframe", "src" : u.data.base_url + 'cat=' + u.data.multicat_arr[f] + ((c.length > 0) ? ';' + c.join(u.data.qsp_delim) : '') + '?', "loc" : "body", "id" : 'utag_136_iframe' });
        }
      }
    };
    utag.o[loader].loader.LOAD(id);
  }('136', 'vmware.microsites-privacy'));
} catch (error) {
  utag.DB(error);
}
